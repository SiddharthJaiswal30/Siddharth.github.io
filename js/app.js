...
function startLoader() {
  // No counting loader functionality needed
}
startLoader();
...
function hideLoader() {
  gsap.to(".loader__wrapper", { duration: 0.8, ease: 'power4.in', opacity: 0, delay: 2.2 });
  setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  contactForm.addEventListener('submit', function(event) {
    event.preventDefault();
    emailjs.sendForm('service_d0y613c', 'template_kixy7m5', contactForm)
      .then(function(response) {
        console.log('EmailJS response:', response);
        alert('Form submitted successfully!');
        contactForm.reset(); // Clear the form data
        formContainer.classList.add('is-hidden'); // Hide the form
        formReply.classList.add('is-visible'); // Show the reply message
        setTimeout(function() {
          formReply.classList.remove('is-visible'); // Hide the reply message after 5 seconds
          formContainer.classList.remove('is-hidden'); // Show the form again
        }, 5000);
      }, function(error) {
  ...
  // Ensure the "Download CV" button works
